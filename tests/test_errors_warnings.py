...
class TestErrorsWarnings:
    ...
    @pytest.mark.parametrize("value, group_name, allow_none, allow_empty", [
    ...
    @pytest.mark.parametrize("values, group_name, allow_none, allow_empty", [
        (None, "groups", False, True),
        (None, "groups", False, False),
        ("", "group", False, False),
        ([], "groups", True, False),
    ...
    def test_error_process_generic_set_1(self, values, group_name, allow_none,
                                         allow_empty):
        with pytest.raises(ValueError):
    ...
    @pytest.mark.parametrize("rescale", [
        "",
        "invalid",
        "minmax",
    ])
    ...
    @pytest.mark.parametrize("features, groups", [
        ("invalid", "all"),
        ("invalid", "general"),
        ("mean", "info-theory"),
        ("nr_instt", "general"),
    ...
    def test_warning_invalid_features(self, features, groups):
        with pytest.warns(UserWarning):
            X, y = load_xy(0)
            model = MFE(
                features=features, groups=groups).fit(
                    X=X.values, y=y.values)
            model.extract()

    @pytest.mark.parametrize("groups, precomp_groups", [
        ("all", "invalid"),
        ("general", "statistical"),
        ("info-theory", "general"),
